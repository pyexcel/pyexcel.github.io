...
function initVM(label) {
    ...
    vm.stdout('Loading '+label+"\n");
    vm.stdout('It\'s big, so this might take a while...\n\n');


    vm.ready().then(function () {
            self.postMessage({cmd: 'reset'});
            vm.stdout('Welcome to '+label+'\n');
            vm.exec('import pickle,time,js;time.sleep=lambda x:[js.globals["Sleep"](x),None][1];');

            vm.exec('import time,js;time.sleep=lambda x:[js.globals["Sleep"](x),None][1];');
            vm.exec('import sys');
            vm.exec('import StringIO');
            vm.exec('import sys,StringIO\nclass fifoFile(object):\n\
                   ...
                   def readline(self):\n\
                     ...
                     self.buffer.read()\n\
                     return line\n\nsys.stdin = fifoFile();stdinwrapper=js.Function(sys.stdin.buffer.write);js.globals["vm"].stdin=stdinwrapper');
            vm.exec('import os;os.fileNumbers[0]=sys.stdin');
            vm.exec('import code;c = code.InteractiveConsole()').then(function () {
                //vm.exec('c.push("""execfile("/lib/pypyjs/lib_pypy/autostart.py")\n""")').then(function () {
...
self.addEventListener('message', function(e){
    ...
    switch (data.cmd){
        case 'execfile':
            if (data.script!=null){
                vm.exec("c.push('''execfile('"+data.script+"')''')");
            }
            break;
        case 'code':
            vm.exec(data.code).then(function(){
