//PyPyDrawThread = (function() {
    ...
    PyPyDrawThread.prototype.initializeVM = function initializeVM(data){
        ...
        vm.ready().then(function() {
            ...
            vm.exec('import multiprocessing');
            ...
            vm.exec('import sys,StringIO\nclass fifoFile(object):\n\
                   ...
                   def readline(self):\n\
                     ...
                     self.buffer.buf=self.buffer.buf[len(line):]\n\
                     self.buffer.read()\n\
                     return line\n\nsys.stdin = fifoFile();stdinwrapper=js.Function(sys.stdin.buffer.write);js.globals["vm"].stdin=stdinwrapper;import os;os.fileNumbers[0]=sys.stdin').then(function(){
                vm.stdin(data);
                vm.exec('import pickle,code');
                vm.exec('c=code.InteractiveConsole();c.push("""import pickle,code,sys;o = pickle.loads(sys.stdin.read());import multiprocessing,multiprocessing.forking;\n""")').then(function(){
