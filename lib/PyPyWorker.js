{
    ...
    PyPyJSWorker.prototype.start=function(label){
        this.worker = new Worker('lib/PyPyMainWorker.js');
        this.worker.postMessage({cmd:'set pypy.vm.js', data:null, label:label});
    ...
    PyPyJSWorker.prototype.stdin=function(txt){
        console.log(133,txt);
        this.worker.postMessage({cmd:'stdin',data:txt});
    };
    PyPyJSWorker.prototype.localStorage=function(data){
        console.log('setting local storage',data.key);
...

{
    function PyPyJS_Subprocess(parent, data) {
        this.parent = parent;
        this.id = data.id;
        this.worker = new Worker('/lib/PyPySubprocess.js');
        this.worker.handler=this;
